...
const SnippetOpen = () => {
    ...
    return (
        <div className={`${isSnippetOpen ? "w-[60%]" : ""}`}>
            <div className={`${isSnippetOpen && isMobileView ? "bg-black/90 z-10 w-screen absolute top-0 left-0 h-screen" : ""}`}></div>
            <SnippetModal />
            <div className={`p-3 rounded-lg ${isMobileView ? "absolute w-[90%] z-20 bg-white/10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" : "h-full w-full bg-white/10"} ${isSnippetOpen ? "block" : "hidden"}`}>
                {singleSnippet && <SnippetHeader singleSnippet={singleSnippet} setSingleSnippet={setSingleSnippet} />}
            </div>
        </div>
    );
}
    ...
    return (
        <>
            <div className='flex justify-between'>
                ...
                <div className='active:bg-white/10 sm:hover:bg-white/10 rounded-full p-2 transition-all duration-150' onClick={() => dispatch(quickLinkAction.setSnippetOpen(!isSnippetOpen))}>
                    <IoMdClose size={25} />
                </div>
            </div>
        </>
